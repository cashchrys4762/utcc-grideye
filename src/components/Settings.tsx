  ...
  return (
    <div style={{ padding: '28px 32px', minHeight: '100vh' }}>
      <div style={{ marginBottom: 28 }}>
        <div style={{ fontSize: 11, color: '#00a3ff', fontFamily: 'JetBrains Mono, monospace', letterSpacing: '0.12em', marginBottom: 4 }}>CONFIGURATION</div>
        <h1 style={{ fontSize: 26, fontWeight: 700, fontFamily: 'Rajdhani, sans-serif', color: '#e2e8f0', margin: 0, letterSpacing: '0.02em' }}>System Settings</h1>
        <div style={{ fontSize: 13, color: '#64748b', marginTop: 4 }}>Manage camera, detection, security, and system preferences</div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
        {/* Camera Configuration */}
        <Card title="Camera Configuration" subtitle="Video stream & encoding parameters">
          <Slider label="Resolution" value={resolution} onChange={setResolution} min={480} max={2160} unit="p" />
          <Slider label="Frame Rate" value={fps} onChange={setFps} min={15} max={60} unit="fps" />
          <Slider label="Bitrate" value={bitrate} onChange={setBitrate} min={2} max={20} unit="Mbps" />
          <div style={{ marginTop: 4 }}>
            <div style={{ fontSize: 12, color: '#475569', fontFamily: 'JetBrains Mono, monospace', marginBottom: 10 }}>STREAM QUALITY PRESET</div>
            <div style={{ display: 'flex', gap: 8 }}>
              {['Economy', 'Balanced', 'High Fidelity'].map((q, i) => (
                <button key={q} style={{
        ...
        <Card title="AI Detection Modules" subtitle="Enable or disable detection algorithms">
          <SettingRow label="Auto-Report Generation" desc="Automatically generate incident reports on detection">
            <Toggle enabled={autoReport} onChange={setAutoReport} />
          </SettingRow>
          <SettingRow label="Red Light Detection" desc="Flag vehicles crossing on red signal">
            <Toggle enabled={redLight} onChange={setRedLight} color="#ef4444" />
          </SettingRow>
          <SettingRow label="Speed Violation Detection" desc="Identify vehicles exceeding speed limits">
            <Toggle enabled={speedDetection} onChange={setSpeedDetection} color="#f59e0b" />
          </SettingRow>
          <SettingRow label="Crowd Density Analysis" desc="Monitor pedestrian density in monitored zones">
          ...
          <div style={{ marginTop: 16, padding: '12px 14px', background: '#0f172a', borderRadius: 8, borderWidth: 1, borderStyle: 'solid', borderColor: '#1e293b' }}>
            <div style={{ fontSize: 12, color: '#475569', fontFamily: 'JetBrains Mono, monospace', marginBottom: 8 }}>ACTIVE MODULES</div>
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {[
                { label: 'Auto-Report', active: autoReport, color: '#00a3ff' },
                { label: 'Red Light', active: redLight, color: '#ef4444' },
                { label: 'Speed', active: speedDetection, color: '#f59e0b' },
                { label: 'Crowd', active: crowdDetection, color: '#a855f7' },
              ].map((m) => (
        ...
        {/* System Info */}
        <Card title="System Information" subtitle="Hardware and software status">
          {[
            { label: 'Platform Version', val: 'GridEye AI v2.4.1' },
            { label: 'Node Count', val: '47 active cameras' },
            { label: 'Model Engine', val: 'YOLOv8-X Turbo' },
            { label: 'GPU Cluster', val: 'NVIDIA A100 × 4' },
            { label: 'Inference Backend', val: 'TensorRT 8.6' },
            { label: 'Uptime', val: '14d 7h 22m' },
            { label: 'Last Model Update', val: '2024-07-08 02:14 UTC' },
            { label: 'Data Region', val: 'Bangkok — ap-southeast-1' },
          ].map((row) => (
          ...
          <div style={{ marginTop: 16, display: 'flex', gap: 10 }}>
            <button style={{ flex: 1, padding: '9px 0', background: 'rgba(0,163,255,0.08)', borderWidth: 1, borderStyle: 'solid', borderColor: 'rgba(0,163,255,0.25)', borderRadius: 8, color: '#00a3ff', fontSize: 13, cursor: 'pointer', fontFamily: 'JetBrains Mono, monospace' }}>CHECK UPDATES</button>
            <button style={{ flex: 1, padding: '9px 0', background: 'rgba(239,68,68,0.08)', borderWidth: 1, borderStyle: 'solid', borderColor: 'rgba(239,68,68,0.2)', borderRadius: 8, color: '#ef4444', fontSize: 13, cursor: 'pointer', fontFamily: 'JetBrains Mono, monospace' }}>RESTART NODE</button>
          </div>
        </Card>

        {/* Security */}
        <Card title="Security & Privacy" subtitle="Data protection and access control">
          <SettingRow label="Auto-Blur License Plates" desc="Anonymize plate numbers in exported footage">
            <Toggle enabled={autoBlur} onChange={setAutoBlur} color="#22c55e" />
          </SettingRow>
          <SettingRow label="Encrypt Activity Logs" desc="AES-256 encryption on all stored logs">
            <Toggle enabled={encryptLogs} onChange={setEncryptLogs} color="#22c55e" />
          </SettingRow>
          <SettingRow label="Two-Factor Authentication" desc="Require 2FA for admin panel access">
          ...
          <div style={{ marginTop: 18, padding: '14px 16px', background: compliant ? 'rgba(34,197,94,0.06)' : 'rgba(245,158,11,0.06)', borderRadius: 8, borderWidth: 1, borderStyle: 'solid', borderColor: compliant ? 'rgba(34,197,94,0.2)' : 'rgba(245,158,11,0.2)' }}>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 600, color: compliant ? '#22c55e' : '#f59e0b', fontFamily: 'JetBrains Mono, monospace' }}>
                {compliant ? 'COMPLIANCE STATUS: PASSED' : 'COMPLIANCE WARNING'}
              </span>
            </div>
            <div style={{ fontSize: 12, color: '#64748b', marginTop: 6, paddingLeft: 16 }}>
              {compliant
                ? 'PDPA Thailand & GDPR Article 25 — Privacy by Design requirements met.'
                : 'Enable plate blur and log encryption to meet PDPA compliance standards.'}
            </div>
          </div>

          <div style={{ marginTop: 16 }}>
            <div style={{ fontSize: 12, color: '#475569', fontFamily: 'JetBrains Mono, monospace', marginBottom: 10 }}>ACCESS CONTROL</div>
            {[
              { role: 'admin@grideye.ai', level: 'Super Admin', last: '18:42 today' },
              { role: 'ops@bangkok.gov', level: 'Operator', last: 'Yesterday' },
              { role: 'viewer@trafficcenter', level: 'Read-only', last: '3 days ago' },
            ].map((u) => (
              <div key={u.role} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottomWidth: 1, borderBottomStyle: 'solid', borderBottomColor: 'rgba(30,41,59,0.5)' }}>
                <div>
                  ...
                  <div style={{ fontSize: 11, color: '#475569', marginTop: 1 }}>Last login: {u.last}</div>
                </div>
                <span style={{ fontSize: 11, fontFamily: 'JetBrains Mono, monospace', padding: '3px 8px', borderRadius: 4, background: 'rgba(0,163,255,0.08)', color: '#00a3ff', borderWidth: 1, borderStyle: 'solid', borderColor: 'rgba(0,163,255,0.2)' }}>{u.level}</span>
              </div>
            ))}
          </div>
        </Card>
